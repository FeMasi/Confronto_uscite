...
if os.path.exists(file_path):
    ...
    clienti_selezionati = st.multiselect("Seleziona uno o più clienti", clienti, default=clienti[:1])
    ...
    df_totale = df[df['MESE'].isin(mesi_selezionati_totali)].groupby('MESE')[['FATTURATO_2024', 'FATTURATO_2025', 'QUANTITA_2024', 'QUANTITA_2025']].sum().reset_index()
    ...
    df_variazione = df[df['MESE'].isin(mesi_selezionati_variazione)]

    # Calcolare la variazione di fatturato in percentuale (con controllo per valori nulli)
    df_variazione['Variazione_Fatt'] = ((df_variazione['FATTURATO_2025'] - df_variazione['FATTURATO_2024']) /
                                        df_variazione['FATTURATO_2024']) * 100

    # Raggruppare per cliente e calcolare la variazione media per cliente
    df_top_clienti = df_variazione.groupby('CLIENTE').agg({'Variazione_Fatt': 'mean'}).reset_index()
    ...
    df_clienti_negativi = df_variazione.groupby('CLIENTE').agg({'Variazione_Fatt': 'mean'}).reset_index()
